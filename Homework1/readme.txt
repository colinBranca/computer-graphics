...
### Exercise 1:  Triangle Spirals

To draw a normal spiral, the constant FERMAT_SPIRAL shall be set to 0.
To draw a Fermat spiral, set it to a non-zero value.

#### Classical Spiral

To generate the first spiral we first noticed that the one in the handout consisted
of around 60 spirals, and that in each "half-spiral" there were around 10 triangles.

We worked in polar coordinates using radius `r` and angle `theta`.
We therefore used a `for` loop from one to sitxy, incrementing the angle by Pi/10.0 for each iteration.
Also, at each iteration, we compute the new radius by multiplying the angle by a positive constant.
...
This gave us the position of each triangle. We set the translation matrix of the triangle
to use these values T[3][0] = x and T[3][1] = y;
...
The formula from the lab slides was used to update the angle theta.

### Exercise 2: Checkerboard
...
We decided to do this because it was easier for us to find the right transformation to obtain
...
We multiplied the coordinates by 10*PI because we wanted to compute a 10x10 checkerboard
(if we wanted a 8x8 chessboard we would have multiplied by 8*PI).
...
For each planet we had to perform three operations:
 ...
 - Translation: to determine the position of the planet (following the orbit equation).
...
To compute the positions of the Moon and the Earth we used the same techniques used in the first exercise:
polar coordinates, ellipse equation (and therefore circle equations).

### Bonus Exercise: Arkanoid
...
Where v2 is the velocity vector after the collision, v1 the velocity vector before
the collision and n the vector normal to the surface of the collision.

To have an angle when the ball bounces against the bar, n was chosen using the following strategy:

 - The horizontal component *nx* is *nx = bcx - bx* where bcx is the horizontal component
   of the position of the center of the bar and bx is the horizontal component of the position of the ball.
 - The vertical component ny is *ny = sqrt(1 - nx^2)*

Various utility functions were added.
